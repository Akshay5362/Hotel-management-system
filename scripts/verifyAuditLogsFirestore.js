import pool from '../backend/db.js';
import { db } from '../backend/config/firebaseAdmin.js';

async function verifyAuditLogs() {
  console.log('=== VERIFYING FIRESTORE /audit_logs COLLECTION ===\n');

  try {
    const [mysqlRows] = await pool.query('SELECT * FROM audit_logs ORDER BY id ASC');
    const firestoreSnap = await db.collection('audit_logs').get();

    console.log(`MySQL Record Count     : ${mysqlRows.length}`);
    console.log(`Firestore Document Count: ${firestoreSnap.size}`);

    if (mysqlRows.length !== firestoreSnap.size) {
      console.error(`COUNT MISMATCH! MySQL: ${mysqlRows.length}, Firestore: ${firestoreSnap.size}`);
      process.exit(1);
    }

    let mismatches = 0;
    for (const row of mysqlRows) {
      const docId = `audit_${row.id}`;
      const docSnap = await db.collection('audit_logs').doc(docId).get();
      if (!docSnap.exists) {
        console.error(`Missing Firestore document: ${docId}`);
        mismatches++;
        continue;
      }
      const data = docSnap.data();
      if (data.action !== row.action) {
        console.error(`Field mismatch in ${docId}`);
        mismatches++;
      }
    }

    console.log(`\nMismatches: ${mismatches}`);
    console.log(`VERDICT: ${mismatches === 0 ? 'AUDIT LOGS MIGRATION VERIFIED' : 'FAILED'}`);
    process.exit(mismatches === 0 ? 0 : 1);

  } catch (err) {
    console.error('Verification error:', err.message);
    process.exit(1);
  }
}

verifyAuditLogs();
