...
async function verifyInventoryCategories() {
  console.log('=== VERIFYING FIRESTORE /inventory_categories COLLECTION ===\n');

  try {
    const [mysqlRows] = await pool.query('SELECT * FROM inventory_categories ORDER BY id ASC');
    const firestoreSnap = await db.collection('inventory_categories').get();
    ...
    for (const row of mysqlRows) {
      ...
      const docSnap = await db.collection('inventory_categories').doc(docId).get();
      if (!docSnap.exists) {
      ...
      if (data.name !== row.name || data.department !== row.department) {
    ...
    console.log(`VERDICT: ${mismatches === 0 ? 'INVENTORY CATEGORIES MIGRATION VERIFIED' : 'FAILED'}`);
