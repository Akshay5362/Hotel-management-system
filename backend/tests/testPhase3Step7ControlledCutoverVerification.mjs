...
async function runVerification() {
  ...
  try {
    ...
    fixturesToClean.push({ collection: 'room_types', docId: `type_${testRtCode}` });
    fixturesToClean.push({ collection: 'room_types', docId: testRtCode });
    fixturesToClean.push({ collection: 'room_types', docId: `type_${testRtCode.toUpperCase()}` });
    ...
    fixturesToClean.push({ collection: 'staff', docId: `staff_${testStaffUser}` });
    fixturesToClean.push({ collection: 'staff', docId: testStaffUser });
    ...
    const firestoreDb = db || (await import('../config/firebaseAdmin.js')).db;
    if (firestoreDb) {
