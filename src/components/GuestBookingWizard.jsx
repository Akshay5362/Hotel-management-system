...
export default function GuestBookingWizard({
  ...
  const taxesAmount = Math.round((baseRate - loyaltyDiscount + servicesTotal) * 0.05);
  ...
  const handleBookSubmit = async () => {
    ...
    const parsedDeposit = paymentDeposit === '' ? 0 : parseInt(paymentDeposit, 10);
    if (paymentMethod !== 'Cash') {
      ...
      if (isNaN(parsedDeposit) || parsedDeposit < 0) {
        showAlert('Invalid deposit amount', 'Validation Error');
        return;
      }
    }
    ...
    try {
      ...
      setConfirmedBooking({
        ...
        cashPendingConfirmation: paymentMethod === 'Cash',
        loyaltyDiscount,
        discountPercent,
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                ...
                <div className="form-group" style={{ marginBottom: '0' }}>
                  <label style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', fontWeight: '500' }}>
                    ...
                    {paymentMethod !== 'Cash' && <>&mdash; <em style={{ color: 'var(--text-muted)', fontSize: '0.75rem' }}>Min ₹ 1,000 required</em></>}
                  </label>
                  <input
                    type="number"
                    min={paymentMethod === 'Cash' ? '0' : '1000'}
                    max={totalStayPrice}
