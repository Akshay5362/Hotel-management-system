...
export default function GuestDashboard({ user, token, rooms, systemDate, onLogout, showAlert, fetchStatus, onUserUpdate }) {
  ...
  useEffect(() => {
    if (prevIsOccupied.current === true && !isOccupied) {
      // Just transitioned out of occupied ΓÇö checkout happened on admin side
      loadGuestHistory();
  ...
  const handleSubmitFeedback = async () => {
    ...
    try {
      ...
      showAlert(`Γ¡É ${data.message}`, 'Review Submitted!');
    } catch (e) {
  ...
  const handleSelfCheckIn = async () => {
    ...
    try {
      ...
      showAlert(`Γ£à ${data.message}`, 'Check-In Successful');
      await fetchStatus();
    } catch (e) {
  ...
  const handleMaintenanceSubmit = async (e) => {
    ...
    try {
      ...
      showAlert('≡ƒöº Maintenance report submitted. Our team will attend shortly.', 'Report Received');
      setMaintenanceIssue('');
    } catch (e) {
  ...
  const handleRequestCheckout = async () => {
    ...
    try {
      ...
      showAlert(`≡ƒôï ${data.message}`, 'Checkout Requested');
    } catch (e) {
  ...
  const FOOD_MENU = [
    ...
    {
      category: '≡ƒìÜ Rice Delights',
      items: [
        { key: 'plain_rice', name: 'Plain Rice', price: 119, photo: '/food/plain_rice.png', desc: 'Steamed premium basmati rice, light, fluffy ΓÇö the perfect accompaniment to any curry' },
    ...
    {
      category: '≡ƒ½ò Vegetarian Mains',
      note: '12 PM ΓÇô 10:30 PM',
      items: [
    ...
    },
    {
      ...
      items: [
        ...
        { key: 'butter_chicken', name: 'Butter Chicken', price: 599, photo: '/food/butter_chicken.png', desc: 'Tender chicken pieces in rich, creamy tomato & butter gravy ΓÇö a classic North Indian favourite' },
    ...
    {
      category: '≡ƒæ¿ΓÇì≡ƒì│ Chef\'s Combos',
      items: [
        { key: 'veg_combo', name: 'Veg Combo', price: 299, photo: '/food/veg_combo.png', desc: 'Paneer Butter Masala + Jeera Rice + 2 Butter Rotis + Pickle ΓÇö a wholesome complete meal' },
        { key: 'non_veg_combo', name: 'Non-Veg Combo', price: 449, photo: '/food/non_veg_combo.png', desc: 'Butter Chicken + Jeera Rice + 2 Tawa Rotis + Pickle ΓÇö tender chicken in creamy gravy' },
      ]
    },
    {
      category: '≡ƒ½ô Breads',
      items: [
        ...
        { key: 'malabar_paratha', name: 'Malabar Paratha', price: 60, photo: '/food/malabar_paratha.png', desc: 'Flaky, layered flatbread with a soft buttery texture, cooked to golden perfection ΓÇö ideal with curries' },
      ]
    },
    {
      category: '≡ƒÑù Salads',
      items: [
        { key: 'fresh_salad', name: 'Fresh Salad', price: 119, photo: '/food/fresh_salad.png', desc: 'Refreshing selection of crisp cucumber, tomatoes, onions & herbs ΓÇö a light & healthy accompaniment' },
    ...
    },
    {
      ...
      items: [
        ...
        { key: 'milk', name: 'Glass of Milk', price: 70, photo: '/food/glass_of_milk.png', desc: 'Pure, fresh milk served hot or chilled ΓÇö a wholesome and nourishing beverage' },
        ...
        { key: 'soft_drinks', name: 'Soft Drinks', price: 70, photo: '/food/soft_drinks.png', desc: 'A selection of refreshing carbonated beverages ΓÇö Coca-Cola / Limca / Seven Up' },
        { key: 'refreshers', name: 'Refreshers', price: 70, photo: '/food/lemon_refresher.png', desc: 'Fresh lemon beverages ΓÇö traditional Indian lemonade or international-style lemon refresher' },
      ]
    },
    {
      category: '≡ƒì« Desserts',
      items: [
        ...
        { key: 'kulfi_stick', name: 'Kulfi (Stick)', price: 99, photo: '/food/kulfi_icecream.png', desc: 'Traditional Indian frozen dessert infused with saffron & cardamom ΓÇö rich, creamy & refreshing' },
    ...
    <div style={{ minHeight: '100vh', background: '#080b10', color: '#f0f6fc', display: 'flex', flexDirection: 'column' }}>
      ...
      <header className="header glass" style={{ borderBottom: '1px solid var(--border-color)', height: '70px', padding: '0 2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div className="brand-section" style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <span className="logo-icon">≡ƒÅ¿</span>
      ...
      {(historyLoading || (activeReservation && !activeBooking)) && (
        <main style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: '16px' }}>
          <div style={{ fontSize: '3rem', animation: 'pulse 1.5s infinite' }}>ΓÅ│</div>
        ...
        <main style={{ flex: 1, padding: '2rem', maxWidth: '900px', width: '100%', margin: '0 auto' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            <div style={{ background: 'linear-gradient(135deg, rgba(251,191,36,0.1) 0%, rgba(245,158,11,0.05) 100%)', border: '1px solid rgba(251,191,36,0.3)', borderRadius: '12px', padding: '16px 20px', display: 'flex', alignItems: 'center', gap: '12px' }}>
              <span style={{ fontSize: '1.5rem' }}>ΓÅ│</span>
              <div>
                <p style={{ fontWeight: '700', color: '#fbbf24', margin: 0, fontSize: '0.95rem' }}>Reservation Confirmed ΓÇö Awaiting Check-In</p>
            ...
            <div className="glass" style={{ borderRadius: '16px', padding: '28px', border: '1px solid rgba(255,255,255,0.07)' }}>
              <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: '1.3rem', fontWeight: '800', color: '#fff', marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                ≡ƒöæ Your Reservation Details
              </h2>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '16px', marginBottom: '28px' }}>
                {[
                  { label: 'BOOKING NO', value: activeReservation?.booking_number || 'ΓÇö' },
                  ...
                  { label: 'CHECK-IN DATE', value: activeReservation?.check_in_date?.split('T')[0] || 'ΓÇö' },
                  { label: 'CHECK-OUT DATE', value: activeReservation?.expected_check_out_date?.split('T')[0] || 'ΓÇö' },
                  { label: 'PAX', value: `${activeReservation?.adults || 1} Guest(s)` },
                  { label: 'DEPOSIT PAID', value: `Γé╣ ${(activeReservation?.advance_amount || 0).toLocaleString('en-IN')}` },
                  { label: 'BASE RATE', value: `Γé╣ ${(liveBill?.booking?.base_rate || 0).toLocaleString('en-IN')} / Night` },
                ].map(({ label, value }) => (
              ...
              <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap', flexDirection: 'column', alignItems: 'center' }}>
                ...
                {paymentStatusInfo?.cashPendingConfirmation && (
                  <div style={{ width: '100%', maxWidth: '520px', background: 'rgba(239,68,68,0.07)', border: '1px solid rgba(239,68,68,0.25)', borderRadius: '12px', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      <span style={{ fontSize: '1.8rem' }}>≡ƒÆ╡</span>
                      <div>
                        ...
                        <p style={{ color: 'var(--text-muted)', margin: '4px 0 0', fontSize: '0.8rem', lineHeight: '1.5' }}>
                          ...
                          <strong style={{ color: '#fff' }}> Γé╣{(paymentStatusInfo.amount || 0).toLocaleString('en-IN')}</strong>.
                    ...
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center', background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.15)', borderRadius: '8px', padding: '10px 14px' }}>
                      <span style={{ fontSize: '0.9rem' }}>Γä╣∩╕Å</span>
                      <p style={{ fontSize: '0.75rem', color: '#94a3b8', margin: 0, lineHeight: '1.4' }}>
                        This page refreshes every 20 seconds. YouΓÇÖll also receive a notification once the staff confirms your payment.
                      </p>
                    </div>
                    ...
                    >
                      ≡ƒöÆ Check In Locked ΓÇö Awaiting Payment
                    </button>
                  </div>
                )}
                ...
                {!paymentStatusInfo?.cashPendingConfirmation && (
                  ...
                  >
                    {isCheckingIn ? 'ΓÅ│ Checking In...' : 'Γ£à Check In Now'}
                  </button>
                )}
              </div>
              <p style={{ textAlign: 'center', fontSize: '0.78rem', color: 'var(--text-muted)', marginTop: '14px' }}>
                {paymentStatusInfo?.cashPendingConfirmation
                  ? '≡ƒÆ╡ Pay your advance deposit at the reception desk. Check-in will unlock once confirmed.'
                  : 'Γä╣∩╕Å Clicking "Check In Now" will confirm your arrival and activate your room. Make sure you are physically at the hotel reception.'}
              </p>
            </div>
          </div>
        </main>
      ...
      {isOccupied && (
        <main style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <div style={{ borderBottom: '1px solid var(--border-color)', background: 'rgba(0,0,0,0.3)', padding: '0 2rem', display: 'flex', gap: '4px', overflowX: 'auto' }}>
            {[
              { id: 'overview', icon: '≡ƒÅá', label: 'Overview' },
              { id: 'service', icon: '≡ƒ¢Ä∩╕Å', label: 'Room Service' },
              { id: 'food', icon: '≡ƒì╜∩╕Å', label: 'Food Order' },
              { id: 'maintenance', icon: '≡ƒöº', label: 'Maintenance' },
              { id: 'bill', icon: '≡ƒôä', label: 'My Bill' },
              { id: 'notifications', icon: '≡ƒöö', label: `Notifications${unreadCount > 0 ? ` (${unreadCount})` : ''}` },
              { id: 'extend', icon: '≡ƒôà', label: 'Extend Stay' },
            ].map(tab => (
              <button
                key={tab.id}
          ...
          <div style={{ flex: 1, overflow: 'auto', padding: '2rem', maxWidth: '1200px', width: '100%', margin: '0 auto' }}>
            ...
            {dashTab === 'food' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                <div>
                  <h2 style={{ fontFamily: 'var(--font-heading)', color: '#fff', fontWeight: '800', fontSize: '1.3rem', marginBottom: '4px' }}>≡ƒì╜∩╕Å In-Room Dining</h2>
                  <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>Order from our kitchen menu. Delivery in 20ΓÇô30 minutes to your room. <span style={{ color: '#fbbf24' }}>Dial 9</span> to order by phone.</p>
                </div>
                      ...
                      {cat.items.map(item => {
                        ...
                        return (
                          <div key={item.key} style={{
                            ...
                            {/* Food photo ΓÇö absolutely positioned in top-right corner */}
                            {item.photo && (
                              <img
                                src={item.photo}
                                alt={item.name}
                            ...
                            {/* Text content ΓÇö padded right to avoid overlap with image */}
                            <div style={{ paddingRight: item.photo ? '74px' : '0' }}>
                              ...
                              <p style={{ color: '#fbbf24', fontWeight: '800', fontSize: '0.85rem', margin: 0 }}>Γé╣ {item.price}</p>
                            </div>
                ...
                <div style={{ position: 'sticky', bottom: 0, background: 'rgba(8,11,16,0.95)', backdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px' }}>
                  <div>
                    ...
                    <p style={{ fontWeight: '800', color: '#fff', fontSize: '1.2rem', margin: 0 }}>Γé╣ {foodCartTotal.toLocaleString('en-IN')}</p>
                  ...
                  >
                    {isPlacingOrder ? 'ΓÅ│ Placing Order...' : '≡ƒì╜∩╕Å Place Order'}
                  </button>
                </div>
              </div>
            )}
            ...
            {dashTab === 'extend' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                <div>
                  <h2 style={{ fontFamily: 'var(--font-heading)', color: '#fff', fontWeight: '800', fontSize: '1.3rem', marginBottom: '4px' }}>≡ƒôà Extend Your Stay</h2>
                ...
                <div className="glass" style={{ borderRadius: '12px', padding: '28px', border: '1px solid rgba(255,255,255,0.07)', maxWidth: '500px' }}>
                  <div style={{ marginBottom: '20px', padding: '16px', background: 'rgba(56,189,248,0.06)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: '8px' }}>
                    ...
                    <p style={{ fontWeight: '800', color: '#38bdf8', fontSize: '1.1rem' }}>{activeReservation?.expected_check_out_date?.split('T')[0] || 'ΓÇö'}</p>
                  </div>
                  ...
                  <form onSubmit={handleExtendStay} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    ...
                    >
                      {isExtending ? 'ΓÅ│ Extending...' : '≡ƒôà Confirm Extension'}
                    </button>
                  </form>

                  <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginTop: '14px', lineHeight: '1.4' }}>
                    Γä╣∩╕Å Extending your stay is subject to room availability. Additional charges will be applied as per your room tariff.
                  </p>
                </div>
              </div>
            )}
      ...
      {!historyLoading && !activeReservation && hasCheckedOut && (
        <main style={{ flex: 1, overflow: 'auto', padding: '0' }}>
          ...
          <div style={{ borderBottom: '1px solid rgba(255,255,255,0.06)', background: 'rgba(0,0,0,0.4)', padding: '0 2rem', display: 'flex', gap: '4px' }}>
            {[
              { id: 'feedback', icon: 'Γ¡É', label: 'Leave a Review' },
      ...
      {showIdReupload && (
        <div className="modal-overlay" style={{ zIndex: 3000 }}>
          <div className="glass" style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <h3 style={{ color: '#f87171', fontWeight: '800', fontSize: '1.1rem', margin: 0 }}>≡ƒôñ Re-upload Identity Document</h3>
              ...
              <button onClick={() => setShowIdReupload(false)} style={{ background: 'transparent', border: 'none', color: '#fff', fontSize: '1.4rem', cursor: 'pointer', lineHeight: 1 }}>├ù</button>
            </div>

            {reuploadSuccess ? (
              <div style={{ textAlign: 'center', padding: '20px' }}>
                <p style={{ fontSize: '2rem', marginBottom: '8px' }}>Γ£à</p>
              ...
              <>
                ...
                <div>
                  ...
                  <label style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', padding: '20px', border: '2px dashed rgba(239,68,68,0.35)', borderRadius: '10px', cursor: 'pointer', background: 'rgba(239,68,68,0.04)', transition: 'all 0.2s' }}>
                    <span style={{ fontSize: '2rem' }}>≡ƒôÄ</span>
                ...
                >
                  {isReuploading ? 'ΓÅ│ Uploading...' : '≡ƒôñ Submit Re-upload'}
                </button>
              </>
            )}
          </div>
